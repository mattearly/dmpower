...
void other_tools()
{
	...
	do
	{
		...
		cout << MAGENTA << "------------OTHER TOOLS------------" << RESET << '\n'
			 ...
			 << " 5. Random Encounter\n"
			 << " 6. NPC Insult\n"
			 << " 7. Back to " << CYAN << "MAIN MENU" << RESET << '\n'
			 << MAGENTA << "-----------------------------------" << RESET << '\n';
		choice = getNumber("Enter Choice: ", 1, max_other_tool_choice);
		switch (choice)
		{
		...
		case 4:
		{
			// todo: make this more than just a chamber test
			Dungeon dungeon;
			mainMessage = dungeon.genChamber();
		}
		break;
		case 5:
		{
	...
	} while (choice < max_other_tool_choice);
